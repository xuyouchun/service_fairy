...
{
    class DimStatementAnalyserStackContext : StatementAnalyserStackContext
    {
        public DimStatementAnalyserStackContext(StatementAnalyserStack analyserStack, IStatementCompileContext context)
            : base(analyserStack, StatementType.Dim, context)
        {
            ...
            if (!_MetAssignOp)  // 等号之前
            {
                ...
                if (varExp != null)
                {
                    if (_ContainsName(varExp.Name))
                        throw new ScriptException(string.Format("变量“{0}”被重复定义", varExp.Name));

                    _Items.Add(new VariableDefineInfo(varExp, null));
                }
                else
                {
                    throw new ScriptException("定义变量时语法错误：" + expression);
                }
            }
            else // 等号之后
            {
                if (_Items.Count == 0 || _Items[_Items.Count - 1].Value != null)
                    throw new ScriptException("DIM语句中等号语法错误");
        ...
        public override bool PushKeyword(Keywords keyword)
        {
            if (keyword == Keywords.Assign)
            {
                ...
                {
                    if (_MetAssignOp)
                        throw new ScriptException("DIM语句中遇到多余的等号");
            ...
            {
                throw new ScriptException("DIM语句中遇到不能识别的关键字“" + KeywordManager.GetKeywordInfo(keyword).Name + "”");
            }
        }

        public override void PushStatement(Statement statement)
        {
            throw new ScriptException("DIM语句声明语法错误");
        }
        ...
        {
            if (_Items.Count == 0)
                throw new ScriptException("DIM语句未指定要声明的变量");

            return new DimStatement(_Items.ToArray());
        }
    }
}
