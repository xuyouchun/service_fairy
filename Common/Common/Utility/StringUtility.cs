...
namespace Common.Utility
{
    ...
    public unsafe static class StringUtility
    {
        ...
        /// <param name="ignoreNull">是否忽略空对象</param>
        /// <returns></returns>
        ...
        /// <param name="ignoreCase"></param>
        /// <returns></returns>
        public static bool EqualsTo(this string s, string s2, bool ignoreCase = false)
        {
            if (!ignoreCase)
                return s == s2;
        ...
        /// <returns></returns>
        public static IDictionary<string, string> KeyValuePairsToDictionary(this string keyValuePairs)
        {
            Dictionary<string, string> dict = new Dictionary<string, string>();
            if (string.IsNullOrEmpty(keyValuePairs))
                return dict;

            fixed (char* fixedP = keyValuePairs)
            {
                char *p = fixedP, pEnd = p + keyValuePairs.Length;
                string key, value;
                while (p < pEnd)
                {
                    if (_ReadKeyValuePairPart(ref p, pEnd, ':', out key) && _ReadKeyValuePairPart(ref p, pEnd, ';', out value))
                        dict[key] = value;
                }
            }
        ...
        private static bool _ReadKeyValuePairPart(ref char* p, char* pEnd, char end, out string value)
        {
            if (!SkipWhitieSpace(ref p))
                goto _false;
            ...
            _found_1:
                if (p >= pEnd)
                    throw new FormatException("键值对字符串格式错误，未找到相匹配的引号");

                value = new string(p0, 0, (int)(p - p0)).Replace(@"\""", "\"").Replace(@"\\", @"\");
                p++;
            }
            else  // 未遇到引号，寻找到结束符
            {
            ...
            _found_2:
                value = new string(p0, 0, (int)(p - p0)).Trim();
            }

            SkipWhitieSpace(ref p);
            if (end == ':')
            {
                if (*p != ':')
                    throw new FormatException("键值对字符串格式错误，键后面必须有冒号");
        ...
        /// <returns></returns>
        public static string ToKeyValuePairsString(this IDictionary<string, string> dict)
        {
            ...
            foreach (KeyValuePair<string, string> item in dict)
            {
                ...
                if (value != null)
                {
                    sb.Append(_ReviseKeyValueStr(key)).Append(":").Append(_ReviseKeyValueStr(value));
                }
            }
        ...
        private static string _ReviseKeyValueStr(string key)
        {
            if (key.IndexOf(';') < 0 && key.IndexOf(':') < 0 && key.Trim() == key)  // 无引号冒号，并无空格前后缀
                return key;
