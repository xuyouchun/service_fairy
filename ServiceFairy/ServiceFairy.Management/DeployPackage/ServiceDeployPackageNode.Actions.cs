    ...
    partial class ServiceDeployPackageNode
    {
        ...
        class DeleteAction : ActionBase
        {
            public override void Execute(IUIObjectExecuteContext context, IServiceObject serviceObject)
            {
                ...
                if (uiOp.ShowQuery("确认要删除该服务安装包吗？") != true)
                    return;

                Kernel._mgrCtx.Invoker.Master.DeleteServiceDeployPackages(new[] { Kernel._packageInfo.Id });
                Kernel.CurrentStatus = ServiceStatus.Deleting;
                Kernel.StartRefresh(delegate {
                    if (Kernel._mgrCtx.ServiceDeployPackageInfos.Exists(Kernel._packageInfo.Id))
