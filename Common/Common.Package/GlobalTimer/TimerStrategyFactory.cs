    ...
    public static class TimerStrategyFactory
    {
        static TimerStrategyFactory()
        {
            var list = from t in typeof(TimerStrategyFactory).Assembly.GetTypes()
                       ...
                       where creator != null
                       select new { Name = attr.Name, Creator = creator };

            _creators = list.ToDictionary(v => v.Name, v => v.Creator);
        }

        private static readonly Dictionary<string, IObjectCreator> _creators;
